// projects/one-thousand-decision-tree/04-video/timing.mjs
...
export const VO_DURATION          = 139.3; // voiceover.mp3 length — must match the actual MP3
...
// matching schedule.
export const HEADER_REVEAL        = 10.5;   // intro ("Six things to know about DCA…")
export const C1_REVEAL            = 33.2;  // c1  ("DCA means investing the same dollars…")
export const C2_REVEAL            = 23.9;  // c2  ("Vanguard and Schwab tested…")
export const C3_REVEAL            = 22.1;  // c3  ("$10,000 invested day one…")
export const C4_REVEAL            = 24.4;  // c4  ("Lump sum earns a full year…")
// Per-project: this project has 4 concepts. C5_REVEAL and C6_REVEAL are
// exported as 0 so TOTAL_REVEAL/COMPOSITION_DURATION sum correctly for the
// inject step (which patches body/root/page-bg data-duration). DO NOT add
// c5/c6 sections without also extending the inline script's durs array and
// COMPOSITION_DURATION expression in index.html.
export const C5_REVEAL            = 0;     // unused (this project has 4 concepts)
export const C6_REVEAL            = 0;     // unused (this project has 4 concepts)
export const OUTRO_REVEAL         = 25.2;  // outro ("That's dollar-cost averaging…")
...
// Per-project: only the concepts that exist in the inline script are
// patched. This project has 4 concepts; C5_REVEAL/C6_REVEAL are exported
// as 0 so the auto-cascade doesn't error if a future build expects 6,
// but they're not listed in INLINE_PATCH_FIELDS.
