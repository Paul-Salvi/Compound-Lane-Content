# 01-text/ — Platform copy for one-thousand-decision-tree

This directory holds the YouTube / Instagram / Twitter copy that ships with
the one-thousand-decision-tree project, ready to paste into each platform at upload time.
...
5. **youtube-problems.txt** — (Stage 3, post-render only) timecoded Q&A
   ...
   `projects/one-thousand-decision-tree/04-video/` has rendered with section timecodes.
...
Run `node scripts/build-text.mjs one-thousand-decision-tree` to regenerate the scaffolding
...
2. Paste the Stage 1 prompt into Claude against
   `projects/one-thousand-decision-tree/01-source/` (Variant B — preferred, no web fetch) or
...
`projects/one-thousand-decision-tree/01-content/one-thousand-decision-tree.json` — the same "every number sourced"
