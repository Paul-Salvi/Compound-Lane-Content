// timing.mjs — Roth IRA Phase-Out MFJ (04-video/)
//
// Single source of truth for the composition's reveal schedule.
// Reel #5 has 6 paragraphs (header + 4 concepts + footer). Word counts
// and estimated VO spans (Paul 1.50× for 6-segment VibeVoice):
//   HEADER:   5 words → ~2.0s → HEADER_REVEAL  2.5
//   C1:      25 words → ~8.6s → C1_REVEAL      9.0
//   C2:      25 words → ~8.6s → C2_REVEAL      9.0
//   C3:      14 words → ~4.8s → C3_REVEAL      5.0
//   C4:      22 words → ~7.6s → C4_REVEAL      7.5
//   OUTRO:    5 words → ~2.0s → OUTRO_REVEAL   2.5
//   sum = 35.5s.  VO_DURATION estimated ~37s native WAV (1.50× atempo).

// ── Tunable constants (the only knobs in the system) ─────────────
export const FLASH_DURATION       = 1.0;
export const FLASH_FADE           = 0.2;
export const VO_DURATION          = 38.03;  // voiceover.mp3 length — measured by ffprobe after regen (1.70× atempo)

export const HEADER_REVEAL        = 2.5;
export const C1_REVEAL            = 9.0;
export const C2_REVEAL            = 9.0;
export const C3_REVEAL            = 5.0;
export const C4_REVEAL            = 7.5;
