...
# VibeVoice (1.5B, Paul voice), applies ffmpeg atempo=1.20, and
...
#          TTS_SPEED          (default: 1.50 — calibrated for 5-6 segment VibeVoice
#                             scripts to land at the 33-37s mid-range of
#                             pacing-rules-v1.md. With 5-6 VibeVoice segments
#                             each costing ~7-10s base, 1.20× under-runs at
#                             ~46s; 1.50× lands at ~34-37s.)
#
# Project: roth-limits-2026/reel-4 (04-video/, audio-only)

set -euo pipefail

cd "$(dirname "$0")"
# 04-video/ is at projects/{parent}/{slug}/04-video/, so repo root is
# 4 levels up. (reel-4 sits one folder deeper than the canonical DCA layout.)
REPO_ROOT="$(cd ../../../.. && pwd)"
