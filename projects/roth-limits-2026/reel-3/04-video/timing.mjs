// timing.mjs — Roth IRA Teen's Three Numbers (04-video/)
//
// Single source of truth for the composition's reveal schedule.
// Reel #3 has 5 paragraphs (header + 3 concepts + footer). Word counts
// and estimated VO spans (Paul 1.20×):
//   HEADER:  5 words → ~2.0s → HEADER_REVEAL  2.5
//   C1:     22 words → ~7.5s → C1_REVEAL      8.0
//   C2:     39 words → ~13.4s → C2_REVEAL    14.0
//   C3:     27 words → ~9.3s → C3_REVEAL     10.0
//   OUTRO:   7 words → ~2.4s → OUTRO_REVEAL   2.5
//   sum = 37.0s.  VO_DURATION estimated ~28s native WAV (1.20×).

// ── Tunable constants (the only knobs in the system) ─────────────
export const FLASH_DURATION       = 1.0;
export const FLASH_FADE           = 0.2;
export const VO_DURATION          = 33.96;  // voiceover.mp3 length — measured by ffprobe after regen (1.50× atempo)

export const HEADER_REVEAL        = 2.5;
export const C1_REVEAL            = 8.0;
export const C2_REVEAL            = 14.0;
