# 01-text/ — Platform copy for emergency-fund-sizing

This directory holds the YouTube / Instagram / Twitter copy that ships with
the emergency-fund-sizing project, ready to paste into each platform at upload time.
...
5. **youtube-problems.txt** — (Stage 3, post-render only) timecoded Q&A
   ...
   `projects/emergency-fund-sizing/04-video/` has rendered with section timecodes.
6. **spanish-subs.srt** — (Stage 4, post-render only, optional) Spanish
   ...
   with a TODO block; filled by `node scripts/build-subs.mjs emergency-fund-sizing` after
...
Run `node scripts/build-text.mjs emergency-fund-sizing` to regenerate the scaffolding
...
2. Paste the Stage 1 prompt into Claude against
   `projects/emergency-fund-sizing/01-source/` (Variant B — preferred, no web fetch) or
...
7. (Optional, Stage 4) After the voiceover timings are measured
   ...
   `node scripts/build-subs.mjs emergency-fund-sizing` to fill **spanish-subs.srt** with
...
`projects/emergency-fund-sizing/01-content/emergency-fund-sizing.json` — the same "every number sourced"
