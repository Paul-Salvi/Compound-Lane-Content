...
// Per-project timing.mjs lives next to the target index.html. Reading
// from there lets each project carry its own measured REVEAL durations
// without coupling to the repo-wide scripts/sound/timing.mjs. The
// build-video.mjs script copies scripts/sound/timing.mjs into
// projects/{slug}/04-video/timing.mjs as a starting point. Fall back
// to the repo-wide timing.mjs (legacy DCA) when the per-project copy
// is missing.
const perProjectTiming = join(dirname(htmlPath), "timing.mjs");
const fallbackTiming = join(__dirname, "timing.mjs");
const timingPath = await readFile(perProjectTiming, "utf8")
  .then(() => perProjectTiming)
  .catch(() => fallbackTiming);
const {
  FLASH_DURATION,
  FLASH_FADE,
  VO_DURATION,
  ...
  INLINE_PATCH_FIELDS,
  MARKUP_PATCH_RULES,
} = await import(pathToFileURL(resolve(timingPath)).href);
...
// place. Anchor on `id="vo"` so the marker is independent of any
// per-project data-hf-id; data-hf-id is now generated by build-video.mjs
// at template-fork time.
const voMarker = 'id="vo"';
