---
preset: code-editorial
format: 1080x1920
layout: portrait
colors:
  bg: "#FAF8F3"
  bg-alt: "#F1EDE2"
  ink: "#1B1F1D"
  ink-muted: "#565A55"
  accent: "#2F5D4E"
  accent-deep: "#223F35"
  accent-active: "#1A3027"
  secondary: "#A85A24"
  surface: "#FFFFFF"
  border: "#DBD5C4"
  danger: "#B00020"
  danger-bg: "#FDF0F0"
fonts:
  display:
    family: "Fraunces"
    weights: [600, 700]
    fallback: Georgia, serif
  mono:
    family: "IBM Plex Mono"
    weights: [400, 600, 700]
    fallback: 'Courier New', monospace
    tabular: true
  body:
    family: "Inter"
    weights: [400, 500, 600]
    fallback: -apple-system, BlinkMacSystemFont, sans-serif
typography:
  display-h1: { size: "52px", weight: 600, color: "#2F5D4E", lineHeight: 1.1, letterSpacing: "-0.015em" }
  display-h2: { size: "36px", weight: 600, color: "#2F5D4E", lineHeight: 1.12, letterSpacing: "-0.01em" }
  stat-hero: { size: "96px", weight: 700, color: "#2F5D4E", lineHeight: 0.82, letterSpacing: "-0.01em" }
  stat-mid: { size: "44px", weight: 700, color: "#2F5D4E", lineHeight: 0.85 }
  stat-muted: { size: "44px", weight: 700, color: "#565A55", lineHeight: 0.85 }
  body: { size: "22px", weight: 400, color: "#565A55", lineHeight: 1.55 }
  label: { size: "14px", weight: 600, color: "#2F5D4E", letterSpacing: "0.08em" }
  caption: { size: "16px", weight: 400, color: "#565A55", lineHeight: 1.5 }
spacing:
  frame-padding: 56px
  logo-top: 40px
  logo-left: 32px
  section-gap: 24px
  card-gap: 16px
  progress-bar-height: 4px
components:
  wordmark:
    width: 38px
    height: 38px
    mark-stroke: "#2F5D4E"
    text-family: "Fraunces"
    text-size: "18px"
    text-weight: 600
    text-color: "#2F5D4E"
  stat-card:
    bg: "#F1EDE2"
    border: "1px solid #DBD5C4"
    border-left: "3px solid #2F5D4E"
    border-radius: 3px
    padding: "16px 20px"
    value-font: "IBM Plex Mono"
    value-size: "24px"
    desc-font: "Inter"
    desc-size: "14px"
  schedule-table:
    border: "1px solid #DBD5C4"
    border-radius: 3px
    cell-padding: "12px 14px"
    header-bg: "#F1EDE2"
    header-font: "IBM Plex Mono"
    header-size: "13px"
    cell-font: "IBM Plex Mono"
    cell-size: "18px"
    pine-cell: "#2F5D4E"
    ochre-cell: "#A85A24"
motion:
  ease-standard: "power2.out"
  ease-entrance: "back.out(1.7)"
  ease-draw: "power2.inOut"
  stat-countup: "sine.out"
