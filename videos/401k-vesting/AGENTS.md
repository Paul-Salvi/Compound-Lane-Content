...
2. Write `BRIEF.md` (YAML frontmatter, 6–8 lines)
3. Write `STORYBOARD.md` (frames × duration — see GUIDE.md for format)
4. Write `SCRIPT.md` (~2.5 words/second at 1.25× speed)
5. Generate TTS: `npx hyperframes tts "..." --voice am_michael --speed 1.25 -o audio/voiceover.wav`
6. Write `index.html` (monolithic for ≤30s — see GUIDE.md for template)
7. `npx hyperframes check` → fix all errors → repeat
8. `npx hyperframes render --fps 30`

### Common pitfalls

- `video-creator` agent fails: "tool permission denied" → Execute the workflow manually
- TTS longer than planned runtime → Re-timeline frames to match actual audio duration
- `content_overlap` layout error → Add `margin-bottom: 48-64px` between stacked text
- "GSAP target #xxx not found" → Delete stale `tl.fromTo` calls for removed elements
- Clips nested inside wrapper `<div>` → Clips MUST be direct children of `#root`
- Missing `class="clip"` → Framework won't hide timed elements
