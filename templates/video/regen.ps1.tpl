...
# from this project's timing.mjs (Windows PowerShell).
#
...
# composition timing is .\timing.mjs (per-project copy of
# scripts\sound\timing.mjs). This script just wires the two together
...
if ($Backend -eq 'vibevoice') {
  ...
  if ($commentLines) {
    Write-Warning "!! tts_script.txt contains $($commentLines.Count) comment line(s) — VibeVoice will treat each as a 'Speaker 1:' segment of dead air. Move editor's notes to a sibling file (e.g. script-notes.md) and strip these from tts_script.txt."
...
# FLASH_FADE, or COMPOSITION_DURATION in .\timing.mjs propagates
...
# .\timing.mjs relative to it (one-line patch in inject-static-sfx.mjs).
Write-Host ""
Write-Host "→ .\timing.mjs → index.html (SFX + timing patch)"
